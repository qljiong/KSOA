...
namespace KSOA.DAL
{
	...
	public partial class Admin_Role
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "Admin_Role"); 
		}
		...
		/// 得到一个对象实体
		/// </summary>
		public KSOA.Model.Admin_Role GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,RoleName,Input,See,Import,Export,RoleNote,IsDelete,AddTime from Admin_Role ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
			...
			KSOA.Model.Admin_Role model=new KSOA.Model.Admin_Role();
		...
		/// 得到一个对象实体
		/// </summary>
		public KSOA.Model.Admin_Role DataRowToModel(DataRow row)
		{
			KSOA.Model.Admin_Role model=new KSOA.Model.Admin_Role();
			if (row != null)
			{
				...
				if(row["Input"]!=null && row["Input"].ToString()!="")
				{
					if((row["Input"].ToString()=="1")||(row["Input"].ToString().ToLower()=="true"))
					{
						model.Input=true;
					}
					else
				...
				if(row["See"]!=null && row["See"].ToString()!="")
				{
					if((row["See"].ToString()=="1")||(row["See"].ToString().ToLower()=="true"))
					{
						model.See=true;
					}
					else
				...
				if(row["Import"]!=null && row["Import"].ToString()!="")
				{
					if((row["Import"].ToString()=="1")||(row["Import"].ToString().ToLower()=="true"))
					{
						model.Import=true;
					}
					else
				...
				if(row["Export"]!=null && row["Export"].ToString()!="")
				{
					if((row["Export"].ToString()=="1")||(row["Export"].ToString().ToLower()=="true"))
					{
						model.Export=true;
					}
					else
				...
				if(row["RoleNote"]!=null)
				{
					model.RoleNote=row["RoleNote"].ToString();
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,RoleName,Input,See,Import,Export,RoleNote,IsDelete,AddTime ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,RoleName,Input,See,Import,Export,RoleNote,IsDelete,AddTime ");
			strSql.Append(" FROM Admin_Role ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
