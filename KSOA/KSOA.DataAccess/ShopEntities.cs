...
using System.Linq;
using System.Text;
using U1City.Shop.Unit;
using log4net;

namespace U1City.Shop.DataAccess
{
    public partial class ShopEntities
    {
        ...
        public int SaveChanges(string method)
        {
            ...
            try
            {
                var modifiedObjects = ObjectStateManager.GetObjectStateEntries(System.Data.EntityState.Modified);
                if (modifiedObjects.Count() > 0)
                    foreach (var modifiedObject in modifiedObjects)
                    {
                        modifiedStr.Append(SerializeHelp.Serialize(modifiedObject.Entity));
                    }
                var addObjects = ObjectStateManager.GetObjectStateEntries(System.Data.EntityState.Added);
                if (addObjects.Count() > 0)
                    foreach (var addObject in addObjects)
                    {
                        addStr.Append(SerializeHelp.Serialize(addObject.Entity));
                    }
                var delObjects = ObjectStateManager.GetObjectStateEntries(System.Data.EntityState.Deleted);
